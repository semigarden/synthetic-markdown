...
function buildAst(text, previousAst = null) {
...
export function buildBlocks(text, previousAst = null) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (state.inFencedCodeBlock) {
            ...
            const currentBlock = blocks.find(b => b.id === state.codeBlockId);
            if (closeMatch) {
                if (currentBlock) {
                    currentBlock.text += "\n" + line;
        ...
        switch (detectedBlock.type) {
            ...
            case "listItem": {
                ...
                if (lastBlock && lastBlock.type === "list" && lastBlock.ordered === !!detectedBlock.ordered) {
                    lastBlock.blocks.push(listItem);
    ...
    const reconcileNested = (blockList) => {
        for (const b of blockList) {
            if ('blocks' in b && b.blocks.length > 0) {
...
function parseInlinesRecursive(block, previousBlock) {
    ...
    switch (block.type) {
        ...
        default:
            if ('blocks' in block && Array.isArray(block.blocks)) {
                const prevNestedMap = new Map();
                if (previousBlock && 'blocks' in previousBlock && Array.isArray(previousBlock.blocks)) {
                    previousBlock.blocks.forEach((b) => {
                        if (b.id)
                            prevNestedMap.set(b.id, b);
                    });
                }
...
function parseInlines(block, previousInlines = []) {
    const next = [];
    ...
    if (block.type === "codeBlock") {
        ...
        const shouldReuse = old && (codeContent.startsWith(oldContent) ||
    ...
    for (const frag of newFragments) {
        ...
        if (frag.type === "text") {
            for (const prev of previousInlines) {
                if (usedIds.has(prev.id) || prev.type !== "text")
                    continue;
...
function parseTableRow(line) {
    ...
    if (cellTexts.length > 0 && cellTexts[0].trim() === "")
        cellTexts.shift();
    if (cellTexts.length > 0 && cellTexts[cellTexts.length - 1].trim() === "")
        cellTexts.pop();
    let cells = [];
...
function extractFencedCodeContent(text, fence) {
    const lines = text.split("\n");
    if (lines.length <= 1)
        return "";
...
export function parseInlineContent(text, blockId, offset = 0) {
    const result = [];
    ...
    const isLeftFlanking = (pos, runLength) => {
    ...
    const isRightFlanking = (pos, runLength) => {
        if (pos === 0)
            return false;
...
function parseLinkReferenceDefinitions(text) {
...
function parseLink(text, start, blockId, offset) {
    // Find matching ]
    let bracketDepth = 1;
    let pos = start + 1;
...
function parseImage(text, start, blockId, offset) {
    // start is at "!"
    if (text[start + 1] !== "[")
        return null;
    // Find matching ]
...
function parseLinkDestinationAndTitle(text, start) {
    let pos = start;
    if (pos >= text.length || text[pos] !== "(")
        return null;
    pos++;
...
function processEmphasis(stack, nodes, blockId) {
    if (stack.length === 0)
        return;
    let current = 0;
    while (current < stack.length) {
        ...
        for (let i = current - 1; i >= 0; i--) {
            ...
            if (opener.type !== closer.type || !opener.canOpen || !opener.active)
                continue;
