...
export function renderBlock(block, container, focusedInlineId = null, beforeBlock = null) {
    ...
    if (block.type === 'codeBlock') {
        const code = el.querySelector('code') || document.createElement('code');
        code.innerHTML = '';
        renderInlines(block.inlines, code, focusedInlineId);
    ...
    if (isNew) {
        if (beforeBlock) {
            const beforeEl = container.querySelector(`[data-block-id="${beforeBlock.id}"]`);
            if (beforeEl) {
                beforeEl.after(el);
            }
            else {
