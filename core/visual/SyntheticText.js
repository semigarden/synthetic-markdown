...
export class SyntheticText extends HTMLElement {
    ...
    addDOM() {
        ...
        div.addEventListener('focusin', (e) => {
            ...
            const symbolicOffset = this.mapSyntheticOffsetToSymbolic(syntheticVisibleLength, inline.text.symbolic.length, syntheticOffset);
        ...
        div.addEventListener('focusout', (e) => {
            if (this.focusedInlineId !== null) {
                const inlineEl = this.syntheticEl?.querySelector(`[data-inline-id="${this.focusedInlineId}"]`);
                if (!inlineEl)
                    return;
            ...
            if (!this.syntheticEl?.contains(e.relatedTarget)) {
                const target = e.target;
                if (!target.dataset?.inlineId)
                    return;
    ...
    onBackspace(e) {
        ...
        if (ctx.inlineIndex === 0 && caretPosition === 0) {
            ...
            if (parentBlock) {
                if (parentBlock.type === 'listItem') {
                    ...
                    if (list && list.type === 'list') {
                        ...
                        if (listItemIndex > 0) {
                            const prevListItem = list.blocks[listItemIndex - 1];
                            ...
                            const newInlines = parseInlineContent(mergedText, prevListItemParagraph.id, prevListItemParagraph.position.start);
            ...
            const currText = ctx.block.inlines
                ...
                .join('');
            const mergedText = prevText + currText;
            const newInlines = parseInlineContent(mergedText, prevBlock.id, prevBlock.position.start);
            if (newInlines.length === 0) {
                newInlines.push({
                    id: uuid(),
                    type: 'text',
    ...
    onInput(e) {
        ...
        const detectedBlockType = this.detectBlockType(newText);
        const blockTypeChanged = detectedBlockType.type !== ctx.block.type ||
    ...
    transformBlock(block, text, type) {
        const flattenedBlocks = this.flattenBlocks(this.engine.ast.blocks);
    ...
    normalizeTextContext(params) {
        ...
        const caretPositionInContext = this.caret.getPositionInInlines(oldInlines, inline.id, caretOffset);
    ...
    applyInlineNormalization(block, result) {
        const { contextStart, contextEnd, newInlines, caretInline, caretPosition } = result;
    ...
    restoreCaret() {
        ...
        const inlineEl = this.syntheticEl?.querySelector(`[data-inline-id="${inlineId}"]`);
        if (!inlineEl) {
        ...
        try {
            ...
            if (inlineEl.childNodes.length > 0 && inlineEl.firstChild instanceof Text) {
                ...
                let currentPos = 0;
                const walker = document.createTreeWalker(inlineEl, NodeFilter.SHOW_TEXT, null);
                let node;
    ...
    mapSyntheticOffsetToSymbolic(syntheticLength, symbolicLength, syntheticOffset) {
    ...
    restoreCaretByTextOffset() {
        const pending = this.caret.pendingTextRestore;
        if (!pending)
            return;
        const blockEl = this.syntheticEl?.querySelector(`[data-block-id="${pending.blockId}"]`);
        if (!blockEl)
            return;
        let acc = 0;
    ...
    flattenBlocks(blocks, acc = []) {
        for (const b of blocks) {
            acc.push(b);
            if ('blocks' in b && b.blocks)
                this.flattenBlocks(b.blocks, acc);
        }
        return acc;
    }
}
